    ...
    JV.popSlider.prototype = {
        ...
        setPopHtml: function(a) {
            ...
            if (d[0] = b.data[a], c.slide_data = d, b.data.length) {
                e = b.isShowNum && 1 != b.data.length ? ' style="display:block"': ' style="display:none"';
                var f = ["{@each slide_data as item,index}", '<div class="w-light-box">', '<div class="head">',
                         "{@if item.title}", '<div class="title">${item.title}</div>', "{@/if}", 
                         '<div class="num"' + e + ">" + (a + 1) + "/" + b.data.length + "</div>", "</div>", 
                         '<div class="content"{@if !item.info} style="height:85%"{@/if}>', "{@if item.pic}",
                         '<img class="pic" src="${item.pic}" alt=""><img class="loading" src="' + b.loading + '" alt="" />',
                         "{@else}", "${item.video|setVideo,item.flv,item.width,item.height}", "{@/if}", '<div class="control"' + e + ">",
                         '<a class="prev" href="javascript:void(0);"></a><a class="next" href="javascript:void(0);"></a>', "</div>",
                         "</div>", "{@if item.info}", '<div class="info">${item.info}</div>',
                         "{@/if}", "</div>",
                         "{@/each}"].join("");
        ...
        setVideo: function(a, b, c, d) {
            if (Modernizr.video) var e = ['<video width="' + c + '" height="' + d + 
                                          '" controls="controls" autoplay="autoplay" preload="auto">', 
                                          '<source src="' + a + '" type="video/mp4">', "您的浏览器暂时无法播放此视频.</video>"];
            else var f = "http://v.163.com/swf/video/NetEaseFlvPlayerV3.swf",
            e = ['<object width="' + c + '" height="' + d + '" id="FPlayer" data="' + f + 
                 '" type="application/x-shockwave-flash">', '<param value="transparent" name="wmode" />',
                 '<param value="true" name="allowFullScreen" />', '<param value="always" name="allowscriptaccess" />',
                 '<param value="' + f + '" allownetworking="all" name="movie" />', '<param value="' + b + '" name="flashvars" />',
                 "</object>"];
            return e
        },
        setSlider: function() {
        ...
        setHandleCallback: function(b) {
            ...
            k = c.data.length,
            l = c.data[b];
            if (null == c.data[0].video) e.attr("src", l.pic),
